...
function topology(){
 const rect=canvas.getBoundingClientRect(),w=rect.width,h=rect.height,dpr=Math.min(devicePixelRatio||1,2);canvas.width=w*dpr;canvas.height=h*dpr;ctx.setTransform(dpr,0,0,dpr,0,0);ctx.clearRect(0,0,w,h);polygons=[];canvas.dataset.view=state.kind==='square'?'square':isCube()?'cube':state.map?'flat':'wrapped';
 ...
 if(state.map){
  ...
  for(let i=0;i<rows;i++)for(let j=0;j<4;j++){
   ...
   if(state.patch&&face===state.face){ctx.save();ctx.strokeStyle='#23343d';ctx.lineWidth=1.5;ctx.setLineDash([3,3]);ctx.strokeRect(x+3,y+3,cell-6,cell-6);ctx.restore();}
  }
  ...
  ctx.setLineDash([]);
  // Same-direction arrows identify opposite sides by translation, not reflection.
  // One arrowhead pairs top/bottom; two pair left/right (also readable without color).
  function arrows(x,y,dx,dy,count,color){
   ctx.strokeStyle=color;ctx.lineWidth=2;
   for(let k=0;k<count;k++){const a=x+(k-(count-1)/2)*7*dx,b=y+(k-(count-1)/2)*7*dy;
    ctx.beginPath();ctx.moveTo(a-4*dx+3*dy,b-4*dy-3*dx);ctx.lineTo(a,b);ctx.lineTo(a-4*dx-3*dy,b-4*dy+3*dx);ctx.stroke();}
  }
  for(const y of [y0,y0+4*cell])arrows(x0+cell*.5,y,1,0,1,'#315f9c');
  for(const x of [x0,x0+rows*cell])arrows(x,y0+cell*.5,0,1,2,'#168477');
  // Repeated boundary labels are the same vertex conic after gluing.
  const selected=data.faces[state.face].vertices;
  const sub=n=>String(n).replace(/\d/g,d=>'₀₁₂₃₄₅₆₇₈₉'[+d]);
  for(let i=0;i<=rows;i++)for(let j=0;j<=4;j++){
   const v=4*(i%rows)+j%4,k=selected.indexOf(v),x=x0+i*cell,y=y0+j*cell;
   ctx.beginPath();ctx.arc(x,y,k<0?2:4,0,2*Math.PI);ctx.fillStyle=k<0?'#78958a':colors[k];ctx.fill();
   if(k<0)continue;
   const label='Q'+sub(i%rows)+','+sub(j%4),tx=x+(i===rows?-6:6),ty=y+(j===0?13:-6);
   ctx.font='11px Georgia';ctx.textAlign=i===rows?'right':'left';ctx.lineWidth=3;ctx.strokeStyle='#f7faf3';ctx.strokeText(label,tx,ty);ctx.fillStyle=colors[k];ctx.fillText(label,tx,ty);
  }
  ctx.textAlign='left';canvas.dataset.camera=JSON.stringify([state.yaw,state.tilt]);return;
 }
...
function selectFace(f,togglePatch=true){
 if(!Number.isInteger(f)||f<0||f>=data.faces.length)return;
 clearTrace();state.face=f;state.edge=-1;state.reveal=false;
 if(state.patch){if(togglePatch){const i=state.region.indexOf(f);if(i>=0)state.region.splice(i,1);else state.region.push(f);}}else state.region=[f];schedule();
}
function render(){
 try{data=isCube()?ConicCube.build(state):isNet()?Net.torus({...state,rows:state.kind==='odd'?3:4}):Net.noncoherent();if(state.kind==='square')state.face=0;
  ...
  $('surface-parameters').hidden=!isNet();$('patch-controls').hidden=!isNet();$('net-map').hidden=!isNet()||isCube();$('cube-patches').hidden=!isCube();$('patch-four').hidden=isCube();
  canvas.style.cursor=!isNet()?'default':!isCube()&&state.map?'pointer':'grab';
  canvas.setAttribute('aria-label',!isNet()?'A single quadrilateral of contacting conics.':isCube()?'Rotatable Penrose cube. Drag to orbit; click a face to inspect its four conics. Arrow keys turn the camera; Page Up and Page Down select faces.':state.map?'Flattened periodic torus map. Opposite sides with matching arrows are identified. Click a face or use arrow keys to inspect its conics. Home and End select the first and last face.':'Rotatable labeled torus. Drag to orbit; click a face to inspect its conics. Arrow keys turn the camera; Page Up and Page Down select faces.');$('net-counts').textContent=isNet()?`${data.Q.length} conics · ${data.edges.length} contacts · ${data.faces.length} faces`:'4 conics · 4 contacts · H = 2';
  $('topology-caption').textContent=!isNet()?'A single face; no gluing is asserted.':isCube()?'The cube is a six-face sphere, as in the Desargues example. Its vertices now carry conics. Click a face, or use Page Up / Down to select one.':state.map?'Opposite sides with matching arrows are identified. Repeated labels are the same conic. Orange marks the patch boundary. Arrow keys select faces; in patch mode, Space toggles the focused face.':'Drag to turn the gluing diagram; click a face. Page Up / Down selects faces. Turn off 3D torus to return to the periodic map.';
  $('net-map').textContent='3D torus';$('net-map').setAttribute('aria-pressed',String(!state.map));$('net-map').title=state.map?'Wrap the periodic map into a torus.':'Return to the flattened periodic map.';
  canvas.setAttribute('aria-description',`Selected face ${state.face+1} of ${data.faces.length}${state.patch?', '+state.region.length+' faces in the patch':''}.`);
  $('reveal-face').textContent=state.reveal?'Hide concurrence':'Reveal concurrence';$('reveal-face').disabled=state.kind==='square';
  // Keep contact buttons in the DOM so keyboard focus survives animation.
...
function cubePatch(faces){
 stop(true);state.kind='cube';state.face=faces[0];state.region=faces.slice();state.patch=true;state.edge=-1;state.reveal=false;state.zoom=1;state.pan=[0,0];
...
canvas.addEventListener('keydown',e=>{
 if(!isNet())return;
 const count=data.faces.length,arrow=['ArrowLeft','ArrowRight','ArrowUp','ArrowDown'].includes(e.key);
 if(['Home','End','PageUp','PageDown'].includes(e.key)){
  e.preventDefault();selectFace(e.key==='Home'?0:e.key==='End'?count-1:(state.face+(e.key==='PageUp'?-1:1)+count)%count,false);return;
 }
 if(state.patch&&[' ','Enter'].includes(e.key)){e.preventDefault();selectFace(state.face);return;}
 if(!arrow)return;e.preventDefault();
 if(!isCube()&&state.map){
  const i=state.face>>2,j=state.face%4;
  const x=(i+(e.key==='ArrowLeft'?-1:e.key==='ArrowRight'?1:0)+data.rows)%data.rows;
  const y=(j+(e.key==='ArrowUp'?-1:e.key==='ArrowDown'?1:0)+4)%4;
  selectFace(4*x+y,false);
 }else{
  state.yaw+=e.key==='ArrowLeft'?-.12:e.key==='ArrowRight'?.12:0;
  state.tilt=Math.max(-1.4,Math.min(1.4,state.tilt+(e.key==='ArrowUp'?.1:e.key==='ArrowDown'?-.1:0)));topology();
 }
});
