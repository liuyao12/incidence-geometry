...
// Depth colors follow Chern's colored all-ellipse plate; other tabs keep their palette.
const PC=['#273b46','#265bad','#7e9ecf','#00897e','#4890df','#348e87','#70a49f','#c25624'];
const state={mode:'penrose',preset:'chern',params:E.defaults(),complete:false,selection:{kind:'all',id:0},chords:true,O:[0.08,-0.05,1],ts:[0.57,0.68,0.78],weights:E.coherentWeights(),t:0.45,angle:0.6,five:G.defaultFive(),theta:.42,envelope:false,playing:false,zoom:1,pan:[0,0]};
...
function render(){try{
 const m=state.mode;$('preset-row').hidden=m!=='penrose';$('conic-preset').value=state.preset;$('penrose-legend').hidden=m!=='penrose';$('preset-note').textContent=state.preset==='chern'?'Arrangement inspired by Chern’s colored ellipse diagram; independently chosen parameters.':'Original nearly nested parameter family, kept for comparison.';$('stage').className=`stage ${m}`;$('complete').hidden=m==='surface'||m==='bridge'||m==='pascal';$('complete').textContent=state.complete?'Hide completion':'Complete cube';$('complete').setAttribute('aria-pressed',String(state.complete));$('chord-toggle').hidden=m!=='penrose';$('overview').hidden=m==='bridge'||m==='pascal';$('inspect').parentElement.hidden=m==='bridge'||m==='pascal';$('envelope-toggle').hidden=m==='surface'||m==='fomin';$('play').hidden=m!=='pascal';$('play').textContent=state.playing?'Pause trace':'Trace F';
 ...
 $('stage-label').textContent={pascal:'Drag A–E · orange F is constructed by joins and meets',penrose:'Drag p₁–p₃ · inspect an edge for its contact chord',fomin:'Filled points / lines · drag O',surface:'A closed, oriented cube surface',bridge:'Gray: q₀ · green: q₀ − tℓ² · dashed: ℓ = 0'}[m];
 ...
 if(m==='penrose')$('completion-key').hidden=!state.complete;
...
$('conic-preset').onchange=()=>{userChange();state.preset=$('conic-preset').value;state.params=E.defaults(state.preset);state.complete=false;state.selection={kind:'all',id:0};state.pan=[0,0];state.zoom=1;controls();populateInspect();render();};
$('inspect').onchange=()=>{userChange();const[k,id]=$('inspect').value.split(':');state.selection={kind:k,id:+id};render();};$('chords').onchange=()=>{userChange();state.chords=$('chords').checked;render();};$('overview').onclick=()=>{userChange();state.selection={kind:'all',id:0};$('inspect').value='all:0';render();};$('complete').onclick=()=>{userChange();state.complete=!state.complete;render();};$('reset').onclick=()=>{userChange();state.params=E.defaults(state.preset);state.five=G.defaultFive();state.theta=.42;state.O=[.08,-.05,1];state.ts=[.57,.68,.78];state.weights=E.coherentWeights();state.complete=false;state.t=.45;state.angle=.6;setMode(state.mode);};
